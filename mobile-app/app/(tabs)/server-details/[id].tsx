...
export default function ServerDetailsScreen() {
  ...
  const fetchServerDetails = async () => {
    try {
      const [serverData, metricsData] = await Promise.all([
        ...
        apiService.getServerMetrics(id as string)
      ]);
      ...
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              const result = await apiService.rebootServer(id as string);
  ...
  const confirmRestartService = async () => {
    ...
    setActionLoading(true);
    try {
      const result = await apiService.restartService(id as string, selectedService);
  ...
  const getChartData = () => {
    ...
    return {
      ...
      datasets: [
        {
          data: last24Hours.map((m: any) => m.cpu_percent || 0),
          color: (opacity = 1) => `rgba(231, 76, 60, ${opacity})`,
          strokeWidth: 2,
        },
        {
          data: last24Hours.map((m: any) => m.ram_percent || 0),
